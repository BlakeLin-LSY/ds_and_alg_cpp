...
Problem
    ...
    For each GBus, the range of cities that it serves is provided:
        the i-th gBus serves the cities with numbers between Ai and Bi, inclusive.
...
Input
    ...
    each case is separated from the next by one blank line. (Notice that this is unusual for Kickstart data sets.)
    ...
    The second line contains 2N integers representing the ranges of cities that the buses serve, 
        in the form A1 B1 A2 B2 A3 B3 ... AN BN. That is, 
        the first GBus serves the cities numbered from A1 to B1 (inclusive), 
        the second GBus serves the cities numbered from A2 to B2 (inclusive), and so on.
...
In Sample Case #1, there are four GBuses. 
    ...
    the third serves cities 45 through 50, and 
    the fourth serves cities 10 through 20. 
    
    City 15 is served by the first and fourth buses, so the first number in our answer list is 2. 
